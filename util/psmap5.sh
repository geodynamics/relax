...
my_gmt(){
	...
	for subprog in $EXTRA; do
		...
		if [ -e "$selfdir/$subprog" ]; then
			echo "# running: $selfdir/$subprog $gset -b $bds -v $VECTOR $OPTIONP -H $HEIGHT $Jset $PROJ $PSFILE"
			eval "$selfdir/$subprog $gset -b $bds -v $VECTOR $OPTIONP -H $HEIGHT $Jset $PROJ $PSFILE"
		else
			if [ -e "$subprog" ]; then
				echo "# running $subprog $gset -b $bds -v $VECTOR $OPTIONP -H $HEIGHT $Jset $PROJ $PSFILE"
...
EOF
	...
	if [ -e "$colors" ]; then
		...
		gmt psscale -O -K -B$PSSCALE/:$unit: -D3.5i/-0.8i/7.1i/0.2ih \
...
#gmt gmtset COLOR_BACKGROUND 255/255/255
#gmt gmtset COLOR_FOREGROUND 0/0/0
...
while [ "$#" != "0" ];do
	...
	# color bounds
	if [ "$pset" == "1" ]; then
		gmt makecpt -C$cptfile -T$P -D > $colors;
