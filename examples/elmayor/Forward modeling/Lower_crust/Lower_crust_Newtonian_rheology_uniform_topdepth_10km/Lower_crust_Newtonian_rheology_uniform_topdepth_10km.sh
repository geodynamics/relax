...
# El Mayor-Cucapah earthquake: Newtonian viscoelastic relaxation in ductile zone in lower crust, uniform top depth of 10 km; bottom depth follows Tape et al [2012] Moho surface
# Note that the grid is 1024^3, which requires >50 GB of RAM on one node. If that exceeds capabilities, try setting sx3 to 512.

#PBS -N Lower_crust_Newtonian_rheology_uniform_topdepth_10km
#PBS -q default
#PBS -l nodes=12
...
WDIR=./Lower_crust_Newtonian_rheology_uniform_topdepth_10km
...
`grep -v "#" ../../../../tape_lowercrust/moho_D10_speed04.dat | wc`
# no dgammadot0 x1  x2 x3 length width thickness strike dip
`grep -v "#" ../../../../tape_lowercrust/moho_D10_speed04.dat`
