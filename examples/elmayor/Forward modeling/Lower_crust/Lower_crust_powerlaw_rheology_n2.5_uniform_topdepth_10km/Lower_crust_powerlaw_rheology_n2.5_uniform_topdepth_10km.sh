...
# El Mayor-Cucapah earthquake: power-law (n=2.5) viscoelastic relaxation in ductile zone in lower crust, uniform top depth of 10 km; bottom depth follows Tape et al [2012] Moho surface
# Note that the grid is 1024^3, which requires >50 GB of RAM on one node. If that exceeds capabilities, try setting sx3 to 512.

#PBS -N Lower_crust_powerlaw_rheology_n2.5_uniform_topdepth_10km
#PBS -q default
#PBS -l nodes=12
...
WDIR=./Lower_crust_powerlaw_rheology_n2.5_uniform_topdepth_10km
...
`grep -v "#" ../../../../tape_lowercrust/moho_D10_speed20000.dat`
