...
# El Mayor-Cucapah earthquake: power-law (n=2.5) viscoelastic relaxation in ductile zone in the lower crust in locations of shallow Lekic et al [2011] lithosphere-asthenosphere boundary; top depth 10 km; bottom depth 22.5 km (Moho depth)
# Note that the grid is 1024^3, which requires >50 GB of RAM on one node. If that exceeds capabilities, try setting sx3 to 512.

#PBS -N Lower_crust_powerlaw_rheology_n2.5_3D_ST_geometry_topdepth10km
#PBS -q default
#PBS -l nodes=12
...
WDIR=./Lower_crust_powerlaw_rheology_n2.5_3D_ST_geometry_topdepth10km
...
# no dgammadot0 x1  x2 x3 length width thickness strike dip
   1    20000 102.5 -28.16 10   400  12.5       125    147  90
   2   -20000 90  -70 10  70   12.5     45    180  90
   3   -20000 110 20 10   50  12.5    100    0   90
   4   -20000 55 130 10  400   12.5    130      147   90
