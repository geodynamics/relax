...
# El Mayor-Cucapah earthquake: power-law (n=4) viscoelastic relaxation in ductile zone in lower crust, uniform thickness of 12.5 km above Tape et al [2012] Moho surface
# Note that the grid is 1024^3, which requires >50 GB of RAM on one node. If that exceeds capabilities, try setting sx3 to 512.

#PBS -N Lower_crust_powerlaw_rheology_n4_uniform_thickness_12kmthick
#PBS -q default
#PBS -l nodes=12
...
WDIR=./Lower_crust_powerlaw_rheology_n4_uniform_thickness_12kmthick
...
`grep -v "#" ../../../../tape_lowercrust/moho_T125_speed10000000.dat`
