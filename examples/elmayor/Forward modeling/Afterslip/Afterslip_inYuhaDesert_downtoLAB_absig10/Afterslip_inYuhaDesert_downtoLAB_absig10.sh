...
# El Mayor-Cucapah earthquake: rate-strengthening afterslip on NW-striking vertical plane in Yuha Desert, extending down approximately to LAB depth (~45-50 km depth); (a-b)sigma = 10 MPa
# Note that the grid is 1024^3, which requires >50 GB of RAM on one node. If that exceeds capabilities, try setting sx3 to 512.

#PBS -N Afterslip_inYuhaDesert_downtoLAB_absig10
#PBS -q default
#PBS -l nodes=12
...
WDIR=./Afterslip_inYuhaDesert_downtoLAB_absig10
...
# nb      x1       x2  x3 length width strike   dip rake
   1    +10.941  -25.585   0.083     24  48    305    90  180
