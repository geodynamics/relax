...
# El Mayor-Cucapah earthquake: rate-strengthening afterslip on downward extensions of planes F2 and F3 in Wei et al [2011] slip model, extending down approximately to LAB depth (~45-50 km depth); (a-b)sigma = 10 MPa
# Note that the grid is 1024^3, which requires >50 GB of RAM on one node. If that exceeds capabilities, try setting sx3 to 512.

#PBS -N Afterslip_beneathrupture_downtoLAB_absig10
#PBS -q default
#PBS -l nodes=12
...
WDIR=./Afterslip_beneathrupture_downtoLAB_absig10
